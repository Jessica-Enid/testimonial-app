...
const C={bg:"#0d0f14",surface:"#13161f",card:"#1a1e2a",border:"#2a2f42",gold:"#c9a84c",goldLight:"#e8c96e",goldSoft:"rgba(201,168,76,0.12)",white:"#f8f6f1",offWhite:"#e8e4da",muted:"#7a7f96",mutedLight:"#9a9fb6",teal:"#2dd4bf"};
const F={display:"'Playfair Display','Georgia',serif",heading:"'Oswald','Arial Narrow',sans-serif",body:"'DM Sans','Helvetica Neue',sans-serif"};
const gStyles=`@import url('https://fonts.googleapis.com/css2?family=Playfair+Display:wght@400;700;900&family=Oswald:wght@400;600;700&family=DM+Sans:wght@300;400;500;600&display=swap');*{box-sizing:border-box;margin:0;padding:0;}body{background:#0d0f14;}@keyframes fadeUp{from{opacity:0;transform:translateY(24px)}to{opacity:1;transform:translateY(0)}}@keyframes shimmer{0%{background-position:-400px 0}100%{background-position:400px 0}}@keyframes pulse{0%,100%{opacity:1}50%{opacity:0.5}}.fade-up{animation:fadeUp 0.5s ease forwards;}.fade-up-1{animation:fadeUp 0.5s 0.1s ease both;}.fade-up-2{animation:fadeUp 0.5s 0.2s ease both;}.fade-up-3{animation:fadeUp 0.5s 0.3s ease both;}`;
const EXAMPLES=[{label:"Coach / Wellness",text:"She was amazing! I really enjoyed working with her and would definitely recommend her to anyone looking for help."},{label:"Consultant / B2B",text:"John is a great consultant. He helped our team a lot and we saw good results. Very professional."},{label:"Creative / Freelance",text:"Loved working with her. Super talented and delivered everything on time. Will hire again for sure."}];
function OutputBlock({label,icon,content,color}){
const [copied,setCopied]=useState(false);
const copy=()=>{navigator.clipboard.writeText(content).then(()=>{setCopied(true);setTimeout(()=>setCopied(false),2000);});};
return(<div style={{background:C.card,border:`1px solid ${C.border}`,borderLeft:`3px solid ${color||C.gold}`,borderRadius:12,padding:"20px 22px",marginBottom:14}}>
<div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
<div style={{display:"flex",alignItems:"center",gap:8}}><span style={{fontSize:16}}>{icon}</span><span style={{fontFamily:F.heading,fontSize:11,fontWeight:"600",letterSpacing:"0.2em",textTransform:"uppercase",color:color||C.gold}}>{label}</span></div>
<button onClick={copy} style={{fontFamily:F.body,fontSize:11,fontWeight:"600",background:"transparent",border:`1px solid ${C.border}`,color:C.muted,padding:"5px 12px",borderRadius:6,cursor:"pointer"}}>{copied?"✓ Copied":"Copy"}</button>
</div>
...
const level=score>=80?{label:"Power Testimonial",color:"#4ade80",bg:"rgba(74,222,128,0.1)"}:score>=55?{label:"Solid Testimonial",color:C.teal,bg:"rgba(45,212,191,0.1)"}:{label:"Weak Testimonial",color:"#f97316",bg:"rgba(249,115,22,0.1)"};
return(<div style={{display:"flex",alignItems:"center",gap:12,marginBottom:20}}>
<div style={{flex:1,height:8,background:C.border,borderRadius:10,overflow:"hidden"}}><div style={{height:"100%",width:`${score}%`,background:level.color,borderRadius:10}}/></div>
<div style={{fontFamily:F.heading,fontSize:11,fontWeight:"600",letterSpacing:"0.15em",textTransform:"uppercase",color:level.color,background:level.bg,padding:"4px 12px",borderRadius:20,whiteSpace:"nowrap"}}>{score}/100 · {level.label}</div>
</div>);
}
...
if(!data.transformed||data.score===undefined) throw new Error("parse");
setResult(data);setActiveTab(0);
}catch(e){
if(e.message==="api") setError("Connection failed. Please try again.");
else if(e.message==="parse") setError("Unexpected response. Please try again.");
...
return(<div style={{background:C.bg,minHeight:"100vh",fontFamily:F.body,color:C.white}}>
<style>{gStyles}</style>
...
{!result&&!loading&&(<div style={{marginTop:8}}>
...
<div key={i} style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:"24px 20px",textAlign:"center"}}>
